...
import { MODBUS_ADDR_AIR_DIOXIDE, MODBUS_ADDR_AIR_HUMID, MODBUS_ADDR_AIR_TEMP, /*MODBUS_ADDR_BOILER_ENABLED,*/ MODBUS_ADDR_BOILER_TEMP, MODBUS_ADDR_BOILER_ENERGY } from "./modbus.js";
...
export class PKOM4Accessory {
	...
	async initAccessories() {	
		...
		this.fanService.getCharacteristic(this.platform.api.hap.Characteristic.On)
			...
			.on(CharacteristicEventTypes.SET, (value: CharacteristicValue, callback: CharacteristicSetCallback) => {
			const fanSwitchedOn = value as boolean;
	...
	dehumidifierManualModeEllapsed() {
		if (this.dehumidifierManualMode) {
			...
			this.dehumidifierManualMode = false;
			this.dehumidifierTargetState = this.platform.api.hap.Characteristic.TargetHumidifierDehumidifierState.HUMIDIFIER_OR_DEHUMIDIFIER;
	...
	serviceAvailabilityChanged() {
		const heaterService = this.accessory.getService(PKOM_BOILER_NAME);
		...
		const purifierService = this.accessory.getService(this.platform.api.hap.Service.AirPurifier);
		...
		const sensorService = this.accessory.getService(this.platform.api.hap.Service.AirQualitySensor);
		...
		const dehumidifierService = this.accessory.getService(this.platform.api.hap.Service.HumidifierDehumidifier);
	...
	const currentWaterHeaterStatus = (this.pkomCurrentlyWaterHeating ? this.platform.api.hap.Characteristic.CurrentHeaterCoolerState.HEATING : this.platform.api.hap.Characteristic.CurrentHeaterCoolerState.IDLE);
	...
	switch (this.pkomMode) {
		...
		this.dehumidifierTargetState = (this.dehumidifierManualMode ? this.platform.api.hap.Characteristic.TargetHumidifierDehumidifierState.DEHUMIDIFIER : this.platform.api.hap.Characteristic.TargetHumidifierDehumidifierState.HUMIDIFIER_OR_DEHUMIDIFIER);
