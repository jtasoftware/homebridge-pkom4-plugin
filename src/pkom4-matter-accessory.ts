...
import { MODBUS_ADDR_VCM_POWER, MODBUS_ADDR_HEAT_POWER, MODBUS_ADDR_ES_PUMP_POWER, MODBUS_ADDR_CR_PUMP_POWER } from "./modbus.js";
import { /*MODBUS_ADDR_VCM_ENERGY, MODBUS_ADDR_HEAT_ENERGY, MODBUS_ADDR_COOL_ENERGY,*/ MODBUS_ADDR_GLOBAL_ENERGY, MODBUS_ADDR_BOILER_ENERGY } from "./modbus.js";
...
// 			this.platform.log.info("Dehumidifier timer elapsed, state is back to " + this.dehumidifierTargetState);
	...
	async loadModbusStatus(keepSession = false) {
		...
		const heatPower = this.session.readRegister(MODBUS_ADDR_HEAT_POWER);
		const firstPumpPower = this.session.readRegister(MODBUS_ADDR_ES_PUMP_POWER);
		const secondPumpPower = this.session.readRegister(MODBUS_ADDR_CR_PUMP_POWER);
