...
export class ModbusSession {
	...
	async install(destination: string): Promise<string | void> {	
	...
	async begin(): Promise<string | void> {
		...
		const promise = await this.callPython(this.pythonVirtualEnv, "modbus.py", "get", this.registersValue)
			.then((result: Record<number, number | string | boolean>) => {
	...
	async callSchellScript(scriptName: string, param: string): Promise<string> {
	...
	async callPython(virtualEnv: string, scriptName: string, verb: string, param?: Record<number, any>): Promise<string> {
		return new Promise(function(successCallback, failureCallback) {
			try {
